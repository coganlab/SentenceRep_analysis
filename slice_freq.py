...
if decompose:
    ...
    all_con = split_and_stack(neural_data_tensor.nanmean(2), -1, 1, 3)
    ...
    all_mask = split_and_stack(mask.any(2), -1, 1, 3)
    ...
    if os.path.exists(filename):
        ...
        losses, model = slicetca.decompose(
            ...
            init_bias=0.1,
            initialization='uniform-positive',
            loss_function=torch.nn.MSELoss(reduction='sum'),#MovingAverageLoss(10),
            # loss_function=SoftDTW(True, 0.001, True, 50,
            #                       torch.nn.MSELoss(reduction='none'),
            #                       # lambda x, y: torch.nn.CosineSimilarity(dim=-1)(x, y).neg().add(1).unsqueeze(-1),
            #                       torch.float16),#, _euclidean_squared_dist),
    ...
    for cond, timing in timings.items():
        ...
        axes = slicetca.plot(mod[..., timing],
                             ...
                             # cmap=parula_map)
    colors = [darken_color('orange', .95), 'k', darken_color('c', 0.9), darken_color('y', 0.9)]
    ...
    W, H = model.get_components(numpy=True)[0]
    W2 = (np.cov(
        model.forward().detach().cpu().numpy().reshape(W.shape[1], -1)) @ W.T).T
    chans = ['-'.join([f"D{int(ch.split('-')[0][1:])}", ch.split('-')[1]]) for
             ch in labels[0]]
    subj = [s for s in layout.get_subjects() if s in set(ch.split('-')[0] for ch in chans)]
    electrode_gradient(layout.get_subjects(), W * 2, chans, colors, mode='both',
                       # fig_dims=(n_components,1)
                       )
    for i in [16]:
        electrode_ratio_gradient(layout.get_subjects(), W, chans, 'brg', 2, i, (2,3))
    ...
    thresh = -6.5
    for mode in ['weights', 'log_weights']:
        if mode == 'weights':
            W_plot = W
            th = np.exp(thresh)
        ...
        for i in range(4):
            for j in range(4):
                ...
                if i == j:
                    ...
                    Wt /= Wt.max()
                    weights = 1 - Wt
                    c_greyed = color_contrast(c, weights)
