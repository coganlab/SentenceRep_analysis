...
def darken_color(color, amount=0.8):
    """Darken a matplotlib color by multiplying its RGB values by 'amount'."""
    rgb = mcolors.to_rgb(color)
    dark_rgb = tuple(np.clip(c * amount, 0, 1) for c in rgb)
    return mcolors.to_hex(dark_rgb)

def autocrop_nonwhite(img, *, white_thresh=250, pad=0, bg="white"):
    """
    ...
    ----------
    img : np.ndarray
        Image array of shape (H, W, 3) or (H, W, 4), dtype uint8.
    white_thresh : int
        Pixels are considered "white background" if all RGB channels >= white_thresh.
        (Higher = stricter; 250 is a good default for screenshots.)
    pad : int
        Extra pixels to include around the detected bbox.
    bg : {"white", "transparent"}
        If "transparent", treats alpha==0 as background too (useful for RGBA screenshots).
    ...
    cropped : np.ndarray
        Cropped image.
    bbox : tuple
        (r0, r1, c0, c1) bbox in original coordinates, where slices are img[r0:r1, c0:c1].
    """
    if img.ndim != 3 or img.shape[2] not in (3, 4):
        raise ValueError("img must have shape (H, W, 3) or (H, W, 4)")

    h, w, ch = img.shape
    rgb = img[..., :3]

    # background mask: True where pixel is background
    bg_mask = np.all(rgb >= white_thresh, axis=2)

    if bg == "transparent" and ch == 4:
        alpha = img[..., 3]
        bg_mask = bg_mask | (alpha == 0)

    # foreground mask: True where pixel is NOT background
    fg = ~bg_mask

    # rows/cols that contain any foreground pixel
    rows = np.any(fg, axis=1)
    cols = np.any(fg, axis=0)

    # If nothing found, return original (or you could return empty)
    ...
    # Convert inclusive indices to slice end-exclusive, then pad & clip
    r0 = max(r0 - pad, 0)
    r1 = min(r1 + 1 + pad, h)
    c0 = max(c0 - pad, 0)
    c1 = min(c1 + 1 + pad, w)
...
def to_plot(mode, W, H, neural_data, timings, i, cond, j = 0, n = 0):
    if mode == 'weights':
        data = H[i, j, ..., timings[cond]]
    elif mode == 'components':
        data = model.construct_single_component(n, i).to(
...
def plot_components(model, mode = 'weights', neural_data = None, plot_latencies = True, n = 0, colors = None):
    ...
    for j, (cond, times) in enumerate(conds.items()):
        ...
        for i in range(n_components):
            plot_dist(
                to_plot(mode, W, H, neural_data, timings, i, cond, n),
        ...
        if j == 0:
            # ax.legend()
            ax.set_ylabel("Z-Score HG power")
...
if decompose:
    ...
    # all_con = movingaverage(all_con.reshape(-1, *all_con.shape[-2:])).reshape(
    #     *all_con.shape[:3], -1, all_con.shape[-1]//4)
    # all_con = all_con.swapaxes(-3, -2)
    ...
    # xlabels = np.linspace(-0.5, 1.5, 200)
    # for n, label in enumerate(labels[0]):
    #     # plot if value in chosen range is in top 20%
    #     # if (examine[i, ..., 90:120] > np.nanpercentile(examine[ ..., 90:120], 99)).any():
    #     #     print(label)
    #     #     ax[n//c, n%c].set_title(label)
    #     #     ax[n//c, n%c].plot(examine[i].detach().cpu().numpy()[..., :200], color='k')
    #     #     ax[n//c, n%c].axvline(50, color='r', linestyle='--')
    #     #
    #     #     # ax[n//c, n%c].set_xticklabels(xlabels)
    #     #     n += 1
    #     ax[n//c, n%c].set_title(label)
    #     ax[n//c, n%c].imshow(examine[n].detach().cpu().numpy(), aspect='auto', origin='lower')
    # raise RuntimeError()
    #
    # all_mask = all_mask.swapaxes(-3, -2)
    # all_mask = movingaverage(all_mask.reshape(-1, *all_mask.shape[-2:])).reshape(all_mask.shape[0], all_mask.shape[1], all_mask.shape[2], -1, all_mask.shape[-1]//2) == 1
    # all_mask = all_mask.swapaxes(-3, -2)

    # all_mask = all_mask.all(axis=2)
    # all_mask = torch.cat([all_mask[..., :150], all_mask[..., 200:]], dim=-1)
    # trial_av = torch.zeros(all_con.shape[:-2] + (all_con.shape[-1],), dtype=torch.float16)
    ...
    if os.path.exists(filename):
        ...
        losses, model = slicetca.decompose(
            ...
            # weight_decay=partial(torch.optim.SGD, momentum=0.8, weight_decay=1e-4),
            # mask=mask,
            mask=all_mask,#.any(2),
            init_bias=0.01,
            initialization='uniform-positive',
    ...
    plt.tight_layout()
    # % plot the model
    idx1 = np.linspace(0, len(labels[0]), 8).astype(int)[1:-1]
    idx2 = np.linspace(0, len(labels[1]), 6).astype(int)[1:-1]
    timings = {'aud_ls': slice(0, 175),
               'go_ls': slice(175, 350),}
    ...
    for cond, timing in timings.items():
        comp = mod.get_components(numpy=True)
        comp[n][0] = comp[n][0][..., timing]
        ...
        axes = slicetca.plot(mod[..., timing],
                             ...
                             sorting_indices=(None,
                                              # None,
                                              np.arange(len(labels[1]))[::-1],
                                              # np.array(labels[2]).argsort()[::-1].astype(float).astype(int),
                                              None),
                             ticks=(None,
                                    # None,
                                    idx2[::-1],
                                    [0, 49, 99, 149]),
                             tick_labels=(None, #[labels[0][i] for i in idx1],
                                          # None,
                                          np.asarray(labels[1])[idx2].astype(float).astype(int),
                                          [-0.5, 0, 0.5, 1]),

                             cmap=parula_map)
    colors = [darken_color('orange', .95), 'k', darken_color('c', 0.9), darken_color('y', 0.9), 'm']
    names = ['Auditory', 'WM', 'Motor', 'Visual', 'Delay']
    ...
    for i, timings in enumerate(timingss):
        for j, (cond, times) in enumerate(timings.items()):
            ...
            ax.set_xlabel("from " + event)

    for ax in axs.flat:
        ax.set_ylim((min(ylims[0]), max(ylims[1])))
    # plt.suptitle(f"Components")
    fig.supylabel('Z-Scored HG Power')
    fig.supxlabel('Time (s)')

    # raise RuntimeError("Stop here")

    # %% plot the region membership
    from ieeg.viz.mri import electrode_gradient, electrode_ratio_gradient, ratio_to_color_gradient
    from itertools import combinations
    W, H = model.get_components(numpy=True)[0]
    W2 = np.log(W + 1e-12)
    W2 -= -6.5
    W2 /= W2.max(1, keepdims=True)
    W2 = np.clip(W2, 0, 1)
    chans = ['-'.join([f"D{int(ch.split('-')[0][1:])}", ch.split('-')[1]]) for
             ch in labels[0]]
    subj = [s for s in layout.get_subjects() if s in set(ch.split('-')[0] for ch in chans)]
    cmaps = [
        LinearSegmentedColormap.from_list(
            "two_color_cmap", [colors[w2], colors[w1]], N=200)
    for j, (w1, w2) in enumerate(combinations(range(4), 2))
        ]
    ...
    if True:
        for i in [16]:
            ...
            brains_6 = electrode_ratio_gradient(layout.get_subjects(), W, chans, cmaps, 2, i, (2,3))

            fig, axs = plt.subplots(2, 3)
            brains = {}

            for j, (w1, w2) in enumerate(combinations(range(4), 2)):
                weights = np.stack([W[w1], W[w2]], axis=0)
                plotter = electrode_ratio_gradient(layout.get_subjects(), weights,
                                                  chans, cmaps[j], 2, i, (1, 1))
                img = plotter.screenshot(return_img=True)
                plotter.close()
                brains[(w1, w2)] = autocrop_nonwhite(img)[0]

                cmap = LinearSegmentedColormap.from_list(
                    "two_color_cmap", [colors[w1], colors[w2]], N=200)
                c = ratio_to_color_gradient(W[w1], W[w2], 'brg', i)
                ax = axs.flat[j]
                ax.scatter(W[w1], W[w2], c=c)
                # ax.set_title(f'{names[w1]} vs {names[w2]}')
                ax.set_xlabel(f'{names[w1]}')
                ax.set_ylabel(f'{names[w2]}')
                ax.set_ylim(0, .5)
                ax.set_xlim(0, .5)

    # bar plots of weight ratios
    fig, axs = plt.subplots(2, 3)
    # thresh = W.max() * 0.01
    thresh = np.percentile(W, 40)
    # thresh = np.exp(-6)
    for j, (w1, w2) in enumerate(combinations(range(4), 2)):
        Wt = W[:, W[w1] + W[w2] > thresh]
        ratios = Wt[w1] / (Wt[w1] + Wt[w2])
        ax = axs.flat[j]
        ax.hist(ratios, bins=3, color='gray', edgecolor='black')
        ax.set_title(f'{names[w2]} <----> {names[w1]})')
        ax.set_xlim(0, 1)
    fig.supylabel('Number of Electrodes')

    # pie chart of co-membership
    fig, axs = plt.subplots(1, 4)
    for i in range(4):
        Wt = W[:, W[i] > thresh]
        membership = np.sum(np.concatenate([Wt[:i], Wt[i+1:]]), axis=1)
        labels_pie = [f'{names[j]}' for j in range(4) if j != i]
        axs[i].pie(membership, labels=labels_pie, autopct='%1.1f%%')
        axs[i].set_title(f'Electrodes in {names[i]}')

    # pie chart of co-membership (categorical)
    fig, axs = plt.subplots(1, 4)
    # thresh = W.max() * 0.1
    for i in range(4):
        Wt = W[:, W[i] > thresh]
        membership = np.sum(np.concatenate([Wt[:i], Wt[i+1:]]) > thresh, axis=1)
        labels_pie = [f'{names[j]}' for j in range(4) if j != i]
        axs[i].pie(membership, labels=labels_pie, autopct='%1.1f%%')
        axs[i].set_title(f'Electrodes in {names[i]}')
    ...
    # fig, axs = plt.subplots(1, 4)
    # for i in range(4):
    #     Wt = np.log(W[:, W[i] > thresh])
    #     corrs = []
    #     labels_pie = []
    #     for j in range(4):
    #         if j != i:
    #             corrs.append(pearsonr(Wt[i], Wt[j])[0])
    #             labels_pie.append(f'{names[j]}')
    #     axs[i].pie(corrs, labels=labels_pie, autopct='%1.1f%%')
    #     axs[i].set_title(f'Electrodes in {names[i]}')
    ...
    def color_contrast(rgba: list[tuple[float, float, float, float]],
                         weights: list[float] = None
                         ) -> list[tuple[float, float, float, float]]:
        """Convert colors to greyscale based on weights.

        Args:
            rgba: List of RGBA colors (values between 0 and 1).
            weights: List of weights (values between 0 and 1) to determine
                the level of greyscale conversion. If None, no conversion is
                applied.

        Returns:
            List of RGBA colors after greyscale conversion.
        """
        if weights is None:
            return rgba
        greyed_colors = []
        grey = 0.5  # Standard luminance conversion
        for color, weight in zip(rgba, weights):
            greyed_color = tuple(
                grey * weight + c * (1 - weight) for c in color[:3]
            ) + (color[3],)
            greyed_colors.append(greyed_color)
        return greyed_colors


    plot_elecs = False
    if plot_elecs:
        # plot electrode groups on brain
        ...
        for j, (w1, w2) in enumerate(combinations(range(4), 2)):
            cmap = LinearSegmentedColormap.from_list(
                "two_color_cmap", [colors[w1], colors[w2]], N=50)
            Wt[0] = W_th[w1] & (~W_th[w2])
            Wt[1] = W_th[w1] & W_th[w2]
            Wt[2] = (~W_th[w1]) & (~W_th[w2])
            Wt[3] = (~W_th[w1]) & W_th[w2]

            col = [colors[w1], cmap(.5), 'lightgrey', colors[w2]]
            fig = electrode_gradient(layout.get_subjects(), Wt, chans, col,
                               mode='both'
                               )
            ...
            for i in range(4):
                r, c = divmod(i, 2)
                for k, (cond, times) in enumerate(timings.items()):
                    ax = axs[r][c][k]
                    plotd = all_con[Wt[i] > 0, 0, ..., timings[cond]].nanmean(
                            axis=1).detach().cpu().numpy()
                    plot_dist(plotd,
                        ax=ax, color=col[i], mode='sem', times=(-0.5, 1.5))
                    ylim = ax.get_ylim()
                    ylims[1].append(ylim[1])
                    ylims[0].append(ylim[0])
                    if j == 1 and i == 2:
    ...
    # 4 x 4 scatter plot of weight correlations
    thresh = -6.5

    for mode in ['log_weights']:
        if mode == 'weights':
            W_plot = W
            th = np.exp(thresh)
        elif mode.startswith('log'):
            W_plot = np.log(W + 1e-10)
            th = thresh
        else:
        ...
        for i in range(4):
            for j in range(4):
                ax = axs[i, j]
                cmap = LinearSegmentedColormap.from_list(
                    "two_color_cmap", [colors[i], colors[j]], N=200)
                if i == j:
                    ax.hist(W_plot[i], bins=20, color=colors[i], edgecolor='black')
                    ax.set_title(f'{names[i]} Weights')
                    ax.axvline(th, color='k', linestyle='--')
                elif i > j:
                    img = brains[(j,i)]
                    ax.imshow(img)
                    ax.axis('off')
                    # ax.set_title(f'Brain: {names[i]} vs {names[j]}')
                else:
                    c = ratio_to_color_gradient(W_plot[i], W_plot[j], colormap=cmap)
                    Wt = np.max([W_plot[i], W_plot[j]], axis=0)
                    Wt -= Wt.min()
                    Wt /= Wt.max()
                    weights = 1 - Wt
                    c_greyed = color_contrast(list(c), list(weights))
                    ax.scatter(W_plot[j], W_plot[i], c=c_greyed, s=4)
                    ax.axvline(th, color='k', linestyle='--')
    ...
    sm_idx = [labels[0].index(l) for l in lzfilled]
    ...
    all_groups = []
    for idx in idxs:
        sm_elecs = [labels[0][i] for i in idx]
    ...
    timings = {'aud_ls': range(0, 175),
                'go_ls': range(200, 375)}
    conds = {'aud_ls': (-0.5, 1.25),
             'go_ls': (-0.5, 1.25)}
    n_comp = W.shape[0]
    fig, axs = plt.subplots(2*len(timings), n_comp)
    data = neural_data_tensor[...,:375].nanmean((1, 2)).detach().cpu().numpy()
    for j, (cond, times) in enumerate(timings.items()):
        j *= 2
        ylims = [0, 0]
        for i, ax in enumerate(axs[0 + j]):
            ...
            sorted_trimmed = trimmed[
                             ...
                             ::-1]
            plot_dist(trimmed.reshape(-1, 175), ax=ax, color=colors[i],
