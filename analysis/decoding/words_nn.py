...
for group in ['SM']:
    model = torch.load(f'model_{group}.pt')
    n_components = model.ranks.sum().item()
    # load the data
    fpath = os.path.expanduser("~/Box/CoganLab")
    sub = GroupData.from_intermediates("SentenceRep", fpath, folder='stats')
    sm_idx = sorted(list(getattr(sub, group)))
    ...
    for epoch, cond_group in epochs.items():
        ...
        for i in range(n_components):
            ...
            data_windowed.labels[2] = np.concatenate([labels[0]] * len(cond_group))
    ...
    # %% save the results
    import pickle
    with open(f'results{group}.pkl', 'wb') as f:
