...
for i, fname in enumerate(fnames):
    ...
    titles = {'aud_ls': 'From Stimulus', 'go_ls': 'From Go Cue', 'resp': 'From Response'}
    ylabels = {"ls": "Z-Score HG power\n (LS)", "lm": "Z-Score HG power\n (LM)", "jl": "Z-Score HG power\n (JL)"}

    # make a plot for each condition in conds as a subgrid
    for k in range(len(conds)):
        for j, cond in enumerate(conds[k]):
            arr = np.nanmean(sub.array['zscore', cond].__array__(), axis=(0, 2))
            ax = axs[2 + k][i][j]
            if conds[k][cond] is not None:
                for group, color, idx in zip(groups[:-1], colors[:-1], idxs[:-1]):
                    plot_dist(arr[idx], times=conds[k][cond],
                              label=group, ax=ax, color=color)
            if j == 0:
                # ax.legend()
                ax.set_ylabel(ylabels[list(conds[k].keys())[0].split('_')[-1]])
                ylims = ax.get_ylim()

            ax.set_ylim(ylims)
            if k == len(conds)-1 and j == 1:
