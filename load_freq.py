...
def load_data(datatype: str, out_type: type | str = float, average: bool = True):
    ...
    if average:
        ...
        # for key in conditions := list(zscore_ave.keys()):
        #     for t in tokens := list(zscore_ave[key].keys()):
        #         for k in channels := list(zscore_ave[key][t].keys()):
        #             for f in frequencies := list(zscore_ave[key][t][k].keys()):
        #                 zscore_ave[key][t][k][f] = zscore_ave[key][t][k][f][..., :200]
        # memmap = np.memmap('data.dat', dtype=out_type, mode='w+',
        #                    shape=(len(conditions), len(tokens), len(channels), len(frequencies), *zscore_ave[key][t][k][f].shape))
        # for i, key in enumerate(conditions):
        #     memmap[i] = LabeledArray.from_dict(zscore_ave[key], dtype=out_type)
