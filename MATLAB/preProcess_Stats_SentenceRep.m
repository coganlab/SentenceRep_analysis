...
RECONDIR='C:\Users\Jakda\Box\ECoG_Recon';
% 
%BOX_DIR='H:\Box Sync';
...
Task.Stats.Field(4).Epoch='Go';
Task.Stats.Field(4).Time=[-1000 -500];
...
% Task.Stats.Baseline.Name='Auditory';
% Task.Stats.Baseline.Epoch='Auditory';
% Task.Stats.Baseline.Time=[-500 0];

Task.Stats.Freq(1).Name='HighGamma';
Task.Stats.Freq(1).Range=[70:150];

Task.Conds(1).Name='LSW';
Task.Conds(2).Name='JLW';
Task.Conds(3).Name='LMW';
Task.Conds(1).Num=1:4;
Task.Conds(2).Num=8:11;
Task.Conds(3).Num=15:18;
    ...
    for iF=1:length(Task.Stats.Field)
        ...
        for iFreq=1:size(Task.Stats.Freq,1)
            ...
            for iChan=1:length(Subject(SN).goodChannels)
                sig1=[];
                sig2=[];
                for iCondN=1:length(Task.Conds(iCond).Num)
                sig1=cat(1,sig1,sq(mean(mean(Field_Spec{Task.Conds(iCond).Num(iCondN)}{iChan}(:,:,Task.Stats.Freq(iFreq).Range),2),3)));
                end
                [goodIdx1 badIdx1]=outlierRemoval(log(sig1),3);
                for iCondN=1:CondParams.Conds
                sig2=cat(1,sig2,sq(mean(mean(Base_Spec{iCondN}{iChan}(:,:,Task.Stats.Freq(iFreq).Range),2),3)));
                end
                [goodIdx2 badIdx2]=outlierRemoval((sig2),3);                
                pVals(iChan,:) = shufflePermTest(sig1(goodIdx1),sig2(goodIdx2),nPerm,nTails);
            end
            save([TASK_DIR '/Stats/' Subject(SN).Name '_' Task.Name '_' Task.Stats.Field(iF).Name '_' Task.Conds(iCond).Name '_' Task.Stats.Freq(iFreq).Name '_' num2str(Task.Stats.nTails) 'Tail_12Outlier.mat'],'pVals');
        end
        end
    end
end
